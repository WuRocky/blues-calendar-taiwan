...
export default defineEventHandler(async (event) => {
  ...
  try {
    const result = await sendWeeklyLinePush({
      ...
      linePublicGroupId: config.lineTestGroupId
    })
    ...
    if (message.startsWith('LINE push failed')) {
      console.error('LINE test weekly push request failed:', message)
    ...
    console.error('Unexpected LINE test weekly push error:', message)

    throw createError({
      statusCode: 500,
      statusMessage: 'Failed to process LINE test weekly push request'
    })
  }
})
