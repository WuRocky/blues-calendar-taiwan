...
const { getOrganizerLogoPath, getOrganizerLogoUrl, getOrganizerLogos, getOrganizerLogoUrls } = jiti('../lib/events/organizerLogo.ts')
...
assert.equal(getOrganizerLogoPath('小白 & Blues20'), '/organizer-logos/blues20.png')
...
assert.deepEqual(
  getOrganizerLogos('Blues20 & TapLife').map(logo => logo.organizer),
  ['Blues20', 'TapLife']
)
assert.deepEqual(
  getOrganizerLogoUrls('Blues20 & TapLife', siteUrl).map(logo => logo.url),
  [
    'https://blues-calendar-taiwan.example.com/organizer-logos/blues20.png',
    'https://blues-calendar-taiwan.example.com/organizer-logos/taplife.png'
  ]
)
...
const combinedOrganizerMessage = formatWeeklyEventsFlexMessage({
  ...
  events: [
    makeEvent({
      ...
      slug: 'combined-organizer',
      eventType: 'social',
      name: 'Combined Organizer Event',
      startTime: '2026-08-08T12:00:00.000Z',
      endTime: '2026-08-08T14:00:00.000Z',
      organizer: 'Blues20 & TapLife'
    })
  ],
  siteUrl
})

assert.equal(combinedOrganizerMessage.contents.type, 'carousel')
...
assert.equal(unknownOrganizerMessage.contents.type, 'carousel')
assert.equal(unknownOrganizerMessage.contents.contents[1].body.contents[3].type, 'box')
assert.equal(unknownOrganizerMessage.contents.contents[1].body.contents[3].contents.length, 1)
assert.equal(unknownOrganizerMessage.contents.contents[1].body.contents[3].contents[0].text, 'Unknown Organizer')
...
assert.match(organizerPreviewApi, /lineChannelAccessToken: config\.lineChannelAccessToken/)
assert.match(organizerPreviewApi, /lineOrganizerGroupId: config\.lineOrganizerGroupId/)
assert.match(organizerPreviewApi, /token: config\.notionToken/)
assert.match(organizerPreviewApi, /databaseId: config\.notionEventsDatabaseId/)
...
assert.match(wranglerConfig, /"0 2 \* \* SUN"/)
assert.doesNotMatch(wranglerConfig, /"0 2 \* \* MON"/)
assert.doesNotMatch(wranglerConfig, /"0 2 \* \* \*"/)
assert.doesNotMatch(wranglerConfig, /"0 2 \* \* 1"/)
assert.match(wranglerConfig, /"\*\/15 \* \* \* \*"/)
assert.match(cronPlugin, /LINE organizer preview cron started/)
assert.match(cronPlugin, /LINE organizer preview cron config status/)
assert.match(cronPlugin, /LINE organizer preview cron push starting/)
assert.match(cronPlugin, /LINE organizer preview cron push sent/)
assert.match(cronPlugin, /NUXT_LINE_ORGANIZER_GROUP_ID/)
assert.match(cronPlugin, /LINE production weekly cron started/)
assert.match(cronPlugin, /LINE production daily cron started/)
assert.match(cronPlugin, /LINE production daily cron config status/)
assert.match(cronPlugin, /LINE production daily cron push starting/)
...
assert.match(cronPlugin, /sendOrganizerPreviewLinePush/)
assert.match(cronPlugin, /sendWeeklyLinePush/)
assert.match(cronPlugin, /sendDailyLinePush/)
assert.match(cronPlugin, /NUXT_LINE_PUBLIC_GROUP_ID/)
assert.match(cronPlugin, /NUXT_LINE_TEST_GROUP_ID/)
assert.match(cronPlugin, /PRODUCTION_WEEKLY_CRON_EXPRESSION = '0 2 \* \* MON'/)
assert.match(cronPlugin, /PRODUCTION_DAILY_CRON_EXPRESSION = '0 2 \* \* \*'/)
assert.match(cronPlugin, /TEST_NEXT_WEEKLY_CRON_EXPRESSION = '0 2 \* \* SUN'/)
